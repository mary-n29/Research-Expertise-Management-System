...
def open_project_window():
    ...
    ttk.Button(
        search_frame,
        text="Search",
        command=lambda: search_projects(
            tree,
    ...
    search_entry.bind(
        "<Return>",
        lambda event: search_projects(tree, search_entry.get().strip())
    )
...
def search_projects(tree, search_text):
    """Filter the project table by name, description, or status."""
